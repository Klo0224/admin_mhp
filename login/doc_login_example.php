  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['signUp'])) {
      ...
      if ($result->num_rows > 0) {
          ...
          if ($upload_ok) {
              $sql = "INSERT INTO doctors (full_name, email, specialization, experience, license_front, license_back, password, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')";
              $stmt = $conn->prepare($sql);
              $stmt->bind_param("sssssss", $full_name, $email, $specialization, $experience, $license_front, $license_back, $password);
